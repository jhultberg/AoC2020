...
for s in data:
    parts =s.strip().split(" ")
    minval, maxval = parts[0].split("-")
    letter = parts[1][0]
    seq = parts[2]
    if int(minval) <= seq.count(letter) <= int(maxval):
        tot += 1
    if bool(seq[int(minval) -1 ] == letter) ^ bool(seq[int(maxval) -1] == letter):
        tot2 +=1
print(tot, tot2)
